...
export type InputValues = {
  [key: string]: string | Array<{ id: string; label: string }>
  checkedHelpless: Array<{ id: string; label: string }>
  checkedUnlovable: Array<{ id: string; label: string }>
  checkedWorthless: Array<{ id: string; label: string }>
  checkedEmotion: Array<{ id: string; label: string }>
}
...
export function DiagramList({ userId, chatId }: DiagramListProps) {
  ...
  const [, setSavedCCDResult] = useState<CCDResult | null>(null)
  const [isSubmitted, setIsSubmitted] = useState(false)
  const [patientType, setPatientType] = useState('')
  const initialInputValues: InputValues = {
    ...Object.fromEntries(
      [...diagramRelated, ...diagramCCD].map(name => [name, ''])
    ),
    checkedHelpless: [],
    checkedUnlovable: [],
    checkedWorthless: [],
      ...
      if (!existingCCDTruth) {
        ...
        const ccdTruth: CCDTruth = {
          ...
          intermediateBeliefDepression:
            patientProfile?.intermediate_belief_depression ?? '',
  ...
  const handleCheckboxChange = (
    category: string,
    checkedValues: Array<{ id: string; label: string }>
  ) => {
    setInputValues(prevValues => ({
      ...prevValues,
  ...
  const handleSubmit = async () => {
    try {
      const ccdResult: CCDResult = {
        ...
        intermediateBeliefDepression: inputValues[
          'intermediateBeliefDepression'
        ] as string,
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between p-4 px-5">
        <h4 className="text-lg font-bold">
          Patient Intake and Cognitive Conceptualization Diagram
        </h4>
      </div>
      <div className="mb-2 px-5 space-y-6 overflow-auto">
            ...
            {name == 'emotion' ? (
              <div className="flex flex-col items-start space-y-2 mt-2">
                {['Emotion'].map((category, index) => (
                  ...
                  >
                    ...
                    />
                    {isSubmitted && (
                      <label className="block leading-normal font-medium text-blue-600">
                        <span className="font-bold">Reference:</span>
                        {savedCCDTruth?.Emotion?.map(
                          (item: string, index: number) => (
                            <div
                              key={index}
                              className="pt-1 leading-normal text-blue-600"
                            >
                              {item}
                            </div>
                          )
              ...
              <div className="flex flex-col items-start space-y-2 mt-2">
                ...
                {isSubmitted && (
                  <label
                    key={name}
                    className="block pt-1 leading-normal font-medium text-blue-600"
                  >
                    <span className="font-bold">Reference: </span>
                    {savedCCDTruth?.[name]}
                  </label>
        ...
        {diagramRelated.map(name => (
          <div key={name}>
            ...
            {name == 'coreBelief' ? (
              <div className="mt-2">
                {['Helpless', 'Unlovable', 'Worthless'].map(
                  (category, index) => (
                    <div
                      className="flex flex-col items-start space-y-2 mt-2"
                      key={category}
                      ...
                      {isSubmitted && (
                        <label className="block leading-normal font-medium text-blue-600">
                          ...
                          {savedCCDTruth?.[category]?.length === 0 ? (
                            <div className="pt-1 leading-normal text-blue-600">
                              not chosen
                            </div>
                          ) : (
                            savedCCDTruth?.[category]?.map(
                              (item: string, index: number) => (
                                <div
                                  key={index}
                                  className="pt-1 leading-normal text-blue-600"
      ...
      <div className="flex justify-end p-4">
        <button
          className="text-sm font-semiboldflex h-[35px] w-[220px] items-center justify-center rounded-md bg-red-500 font-semibold text-white"
