...
import {
  patientTypes,
  patientTypeDescriptions
} from '@/app/api/data/patient-types'
...
interface PatientTypeListProps {
  readonly typeList: Array<string>
  selectedType?: string
  handleChoiceClick: (choice: string) => void
}

const PatientTypeDropdownList: React.FC<PatientTypeListProps> = ({
  typeList,
  selectedType,
  handleChoiceClick
}) => {
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent
        sideOffset={8}
        align="start"
        className="focus:outline-none] absolute z-10 mt-2 w-56 origin-top-right rounded-md shadow-lg ring-1 ring-black ring/5"
      >
...
export function PatientTypeMenu({
  ...
  const patientTypeListValues: Array<string> = patientTypes.map(
    ({ type }) => type
  )
  ...
  const handleChoiceClick = (choice: string) => {
    ...
    const typeDescription = patientTypeDescriptions.find(
      description => description.type === choice
    )
    if (typeDescription) {
      ...
      {isStarted ? (
        ...
        <div>
          <p className="leading-normal pt-4 font-medium text-zinc-500">
            In this CBT session, you will talk to a client simulated by AI with
            a virtual patient profile. You goal is to indentify the cognitive
            conceptualization diagram of the client by communicating with them
            and using CBT skills.
          </p>

          <p className="leading-normal pt-4 font-medium text-zinc-500">
            We provide 5 typical client types and one plain client without any
            types. Please select a patient type to see the description.
          </p>
          <div className="max-w-6xl px-0">
            <div>
              <label className="block pt-4 text-sm font-medium leading-6">
